...
st.title("💎 IF Crystal 全雲端系統 (v10.1 修正版)")
...
if page == "📦 庫存與進貨":
    ...
    with tab_r: # 補貨
        ...
        if not inv.empty:
            ...
            with st.form("r_form"):
                ...
                qty = c1.number_input("進貨數量", 1, value=1, key="r_qty")
                price = c2.number_input("本次進貨總價", 0.0, key="r_price")
                mode = c3.radio("補貨方式", ["➕ 合併", "📦 新批號"], key="r_mode")
    ...
    with tab_c: # 建檔
        ...
        with st.form("c_form"):
            ...
            if st.form_submit_button("✅ 建立商品"):
                fn = n_man if n_sel == MANUAL else n_sel
                fsu = su_man if su_sel == MANUAL else su_sel
                if not fn or not fsu: st.error("名稱與廠商必填"); st.stop()
                new_item = {"編號": f"ST{uuid.uuid4().hex[:8].upper()}", "批號": "初始存貨", "倉庫": wh, "分類": cat, "名稱": fn, "形狀": (sh_man if sh_sel == MANUAL else sh_sel), "五行": (el_man if el_sel == MANUAL else el_sel), "寬度mm": w, "長度mm": l, "進貨數量(顆)": q, "庫存(顆)": q, "成本單價": round(p/q, 2), "進貨廠商": fsu, "進貨日期": str(date.today())}
    ...
    with tab_e: # 修改
        ...
        if not inv.empty:
            ...
            with st.form("e_form"):
                c1, c2, c3 = st.columns(3)
                en = c1.text_input("品名", erow["名稱"])
                ew = c2.number_input("寬度", value=float(erow["寬度mm"]))
                el = c3.number_input("長度", value=float(erow["長度mm"]))
                eq = c1.number_input("修正庫存", value=int(erow["庫存(顆)"]))
                ep = c2.number_input("修正成本", value=float(erow["成本單價"]))
                
                if st.form_submit_button("💾 儲存修改"):
                    st.session_state["inventory"].at[eidx, "名稱"] = en
    ...
    oid = c1.text_input("單號", f"DES-{date.today().strftime('%Y%m%d')}")
    ...
    if st.button("⬇️ 加入清單"):
        st.session_state["current_design"].append({"idx": ridx, "名稱": row["名稱"], "規格": format_size(row), "顏色": row["五行"], "數量": qty, "單價": row["成本單價"], "批號": row["批號"]})
        st.rerun()
    ...
    if st.session_state["current_design"]:
        ...
        for i, item in enumerate(st.session_state["current_design"]):
            total_p += item["單價"] * item["數量"]
            cc1, cc2 = st.columns([5, 1])
            cc1.write(f"🔸 [{item['顏色']}] {item['名稱']} ({item['規格']}) x{item['數量']} | {item['批號']}")
            if cc2.button("🗑️", key=f"del_{i}"):
                st.session_state["current_design"].pop(i)
                st.rerun()
        
        if st.session_state["admin_mode"]: st.metric("預估成本", f"${total_p:.2f}")
        
        if st.button("✅ 確認領出 (扣庫存)", type="primary"):
