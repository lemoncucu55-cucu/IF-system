...
# --- 3. 功能分頁 ---
tab1, tab2, tab3, tab4 = st.tabs(["🧮 作品設計扣量", "📥 入庫與盤點修正", "🔍 庫存查詢與報表", "📤 複製貼上更新數據"])
...
with tab4:
    ...
    st.info("這是最穩定的方法。解決所有亂碼與格式錯誤。")
    
    with st.expander("👉 點此查看操作教學 (3秒完成)", expanded=True):
        st.markdown("""
        1. 在電腦打開您的 **Excel** 檔案。
        2. 全選您要更新的資料（**包含第一行的標題**：倉庫、名稱、庫存...）。
        3. 按下複製 (**Ctrl+C** 或 **Cmd+C**)。
        4. 在下方白色框框點一下，按下貼上 (**Ctrl+V** 或 **Cmd+V**)。
        5. 按下 **Ctrl + Enter** 或點擊空白處，確認下方預覽無誤後，點擊按鈕更新。
        """)

    # 1. 建立一個大文字框讓您貼上
    paste_data = st.text_area("請在此貼上 Excel 資料", height=300, placeholder="請在此點擊並按下 Ctrl+V 貼上...")
        ...
        try:
            ...
            if not missing:
                ...
                if st.button("⚠️ 確認覆蓋系統數據"):
                    ...
                    st.success("數據更新成功！系統將在 2 秒後重整...")
                    time.sleep(2)
                    st.rerun()
            else:
                st.error(f"❌ 讀取失敗。您的資料缺少關鍵欄位：{required}")
                st.write("目前讀到的欄位：", list(df_new.columns))
                st.warning("請確認您複製的 Excel 範圍包含「標題列」。")
                
        except Exception as e:
            st.error(f"資料解析錯誤：{e}")
            st.warning("請確保您是直接從 Excel 表格複製內容。")
