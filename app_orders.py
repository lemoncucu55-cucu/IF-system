...
@st.cache_data(ttl=60)
def load_inventory():
    return _load_sheet("Inventory", INVENTORY_COLUMNS)

@st.cache_data(ttl=60)
def load_history():
    return _load_sheet("History", HISTORY_COLUMNS)

@st.cache_data(ttl=60)
def load_order_items():
    return _load_sheet("OrderItems", ORDER_ITEMS_COLUMNS)

def save_inventory(df):
    _save_sheet("Inventory", df)
    load_inventory.clear()

def save_history(df):
    _save_sheet("History", df)
    load_history.clear()

def save_order_items(df):
    _save_sheet("OrderItems", df)
    load_order_items.clear()

def get_order_items(order_id: str, items_df: pd.DataFrame) -> pd.DataFrame:
...
def calc_order_material_cost(order_id: str, items_df: pd.DataFrame) -> float:
...
def return_inventory_item(order_id, item_row, inv_df, items_df, hist_df):
    ...
    # 2. 移除訂單用料
    mask = (
        (items_df["訂單編號"] == order_id) &
        (items_df["庫存編號"] == inv_id) &
...
# ==========================================
    ...
    if orders_df.empty:
        ...
        DONE_STATUS   = ["已完成", "已取消"]
        ACTIVE_STATUS = [s for s in orders_df["狀態"].unique() if s not in DONE_STATUS]
...
# ==========================================
    ...
    if orders_df.empty:
        st.info("目前沒有任何訂單，請先建立訂單。")
    elif inv_df.empty:
        st.warning("庫存表（Inventory）為空，請先匯入庫存資料。")
    else:
        # ── 選擇訂單 ──
        DONE_STATUS = ["已完成", "已取消"]
        active_orders = orders_df[~orders_df["狀態"].isin(DONE_STATUS)].copy()
        all_order_ids = orders_df["訂單編號"].tolist()

        st.subheader("1️⃣ 選擇訂單")
        show_all = st.checkbox("顯示所有訂單（含已完成/已取消）", value=False)
        if show_all:
            sel_pool = orders_df
        else:
            sel_pool = active_orders

        if sel_pool.empty:
            st.info("沒有可操作的訂單。")
        else:
            sel_pool_display = sel_pool.apply(
                lambda r: f"[{r['狀態']}] {r['訂單編號']} — {r['客戶名稱']} | {r['商品種類']} {r['客製品項']}", axis=1)
            sel_disp = st.selectbox("選擇訂單", sel_pool_display.tolist()[::-1], key="pick_order_sel")
            sel_idx  = sel_pool[sel_pool_display == sel_disp].index[0]
            sel_order = orders_df.loc[sel_idx]
            order_id = sel_order["訂單編號"]
            ...
            with st.container(border=True):
                ...
                m1.metric("訂單", order_id)
                m2.metric("客戶", safe_get(sel_order, "客戶名稱"))
                m3.metric("品項", f"{safe_get(sel_order,'商品種類')} {safe_get(sel_order,'客製品項')}")
                m4.metric("狀態", safe_get(sel_order, "狀態"))
                m5.metric("喜神/忌神", f"{safe_get(sel_order,'喜神') or '-'} / {safe_get(sel_order,'忌神') or '-'}")

            # ── 已領料清單 ──
            st.divider()
            st.subheader("2️⃣ 已領用料清單")
            ...
            if order_items.empty:
                ...
                display_items = order_items[["庫存編號","名稱","五行","形狀","尺寸mm","數量","成本單價","小計","操作人","領料時間"]].copy()
                st.dataframe(display_items, use_container_width=True, hide_index=True)

                material_cost = order_items["小計"].apply(lambda x: _safe_float(x)).sum()
                item_count    = order_items["數量"].apply(lambda x: _safe_float(x)).sum()
                shipping      = _safe_float(safe_get(sel_order, "運費"))
                total_cost    = material_cost + shipping
                sell_price    = _safe_float(safe_get(sel_order, "總售價"))

                sc1, sc2, sc3, sc4, sc5 = st.columns(5)
                sc1.metric("用料品項", f"{len(order_items)} 種")
                sc2.metric("用料顆數", f"{item_count:.0f} 顆")
                sc3.metric("材料成本", f"${material_cost:,.0f}")
                sc4.metric("總成本", f"${total_cost:,.0f}", help="材料成本 + 運費")
                sc5.metric("利潤", f"${sell_price - total_cost:,.0f}")

                # ── 退料 ──
                st.divider()
                st.markdown("##### 🔄 退料")
                ret_opts = [
                    f"{r['庫存編號']} | {r['名稱']} {r['形狀']} {r['尺寸mm']}mm × {r['數量']}顆 ({r['領料時間']})"
                    for _, r in order_items.iterrows()
                ]
                ret_sel = st.selectbox("選擇要退回的品項", ret_opts, key="return_sel")
                if st.button("↩️ 確認退料", key="return_btn"):
                    ret_idx_in_oi = ret_opts.index(ret_sel)
                    ret_row = order_items.iloc[ret_idx_in_oi]
                    inv_df, items_df, hist_df = return_inventory_item(
                        order_id, ret_row, inv_df, items_df, hist_df)
                    save_inventory(inv_df)
                    save_order_items(items_df)
                    save_history(hist_df)
                    # 重算成本回寫訂單
                    new_mat_cost = calc_order_material_cost(order_id, items_df)
                    orders_df.loc[sel_idx, "成本"]   = str(new_mat_cost)
                    orders_df.loc[sel_idx, "總成本"] = str(new_mat_cost + _safe_float(safe_get(sel_order, "運費")))
                    save_orders(orders_df)
                    st.success(f"✅ 已退回：{ret_row['名稱']} × {ret_row['數量']} 顆")
                    time.sleep(1); st.rerun()

            # ── 領料區 ──
            st.divider()
            st.subheader("3️⃣ 從庫存領料")

            # 搜尋/篩選庫存
            fc1, fc2, fc3, fc4 = st.columns(4)
            search_name = fc1.text_input("搜尋名稱", key="inv_search_name")
            wuxing_list = sorted(inv_df["五行"].unique().tolist())
            filter_wx   = fc2.selectbox("篩選五行", ["全部"] + wuxing_list, key="inv_filter_wx")
            shape_list  = sorted(inv_df["形狀"].unique().tolist())
            filter_shape = fc3.selectbox("篩選形狀", ["全部"] + shape_list, key="inv_filter_shape")
            only_stock   = fc4.checkbox("只顯示有庫存", value=True, key="inv_only_stock")

            filtered_inv = inv_df.copy()
            if search_name:
                filtered_inv = filtered_inv[
                    filtered_inv["名稱"].str.contains(search_name, case=False, na=False)]
            if filter_wx != "全部":
                filtered_inv = filtered_inv[filtered_inv["五行"] == filter_wx]
            if filter_shape != "全部":
                filtered_inv = filtered_inv[filtered_inv["形狀"] == filter_shape]
            if only_stock:
                filtered_inv = filtered_inv[
                    filtered_inv["庫存(顆)"].apply(lambda x: _safe_float(x) > 0)]

            if filtered_inv.empty:
                st.info("沒有符合條件的庫存品項。")
            else:
                show_cols = ["編號","名稱","五行","形狀","寬度mm","庫存(顆)","成本單價","倉庫"]
                st.dataframe(
                    filtered_inv[show_cols].reset_index(drop=True),
                    use_container_width=True, hide_index=True)
                st.caption(f"共 {len(filtered_inv)} 筆品項")

                # 領料表單
                with st.form("pick_form"):
                    st.markdown("##### 選擇品項領料")
                    pick_ids = filtered_inv["編號"].tolist()
                    pick_labels = [
                        f"{r['編號']} | {r['名稱']} | {r['形狀']} {r['寬度mm']}mm | 庫存:{r['庫存(顆)']} | ${r['成本單價']}"
                        for _, r in filtered_inv.iterrows()
                    ]
                    pick_sel = st.selectbox("選擇庫存品項", pick_labels, key="pick_item_sel")
                    pc1, pc2 = st.columns(2)
                    pick_qty = pc1.number_input("領料數量（顆）", min_value=1, value=1, step=1, key="pick_qty")
                    pick_operator = pc2.selectbox("操作人", ["Imeng", "千畇"], key="pick_operator")

                    if st.form_submit_button("📥 確認領料", use_container_width=True):
                        pick_idx_in_list = pick_labels.index(pick_sel)
                        pick_inv_row = filtered_inv.iloc[pick_idx_in_list]

                        inv_df, items_df, hist_df, err = pick_inventory_item(
                            order_id, pick_inv_row, pick_qty, pick_operator,
                        ...
                        if err:
                            ...
                            save_orders(orders_df)
                            st.success(
                                f"✅ 已領料：{pick_inv_row['名稱']} × {pick_qty} 顆 "
                                f"→ 訂單 {order_id}")
                            time.sleep(1); st.rerun()

            # ── 此訂單相關歷史紀錄 ──
            st.divider()
            st.subheader("4️⃣ 此訂單進出庫紀錄")
            order_hist = hist_df[hist_df["單號"] == order_id] if not hist_df.empty else pd.DataFrame()
            if order_hist.empty:
                st.info("此訂單尚無進出庫紀錄。")
            else:
